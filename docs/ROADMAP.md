...
| 2026-09-25 | хакатон (LLM-судья) | **LLM-оценка реплик поверх эвристики**: `domain/services/judge.rs` (system-prompt судьи, парсинг JSON 0–10 с обрезкой и разбором markdown, `build_prompt` со сценарием и хвостом диалога); `scoring::{TurnPoints, heuristic_points, blend, apply_points}` — `round((1−w)·эвристика + w·LLM)`, w = `scoring.llm_judge_weight` (default 0.4), бонусы считаются эвристикой; запрос судьи идёт **параллельно** ответу собеседника (`tokio::join!`), сбой любой природы (настройка выкл., квота, модель не настроена, таймаут, мусорный ответ) → откат на эвристику и `judge: null`; настройки `scoring.llm_judge_enabled`/`scoring.llm_judge_weight` в DEFAULTS и в админке; UI-бейдж `LLM s/a/t` в ленте диалога (ru/en i18n); DOCUMENTATION/ROADMAP обновлены |
