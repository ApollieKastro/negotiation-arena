...
| # | Задача | Статус |
|---|--------|--------|
| 1 | Презентация | `[x]` `docs/presentation.pptx` + `docs/presentation.pdf` (11 слайдов, ТЗ п.4) |
...
| 10 | Оценка реплик LLM-моделью | `[x]` `judge.rs` — судья 0–10 (стратегия/аргумент/тон), смешивание 60/40 с эвристикой, откат на эвристику при сбое |
...
| 2026-09-25 | хакатон (LLM-судья) | **LLM-оценка реплик поверх эвристики**: `domain/services/judge.rs` (system-prompt судьи, парсинг JSON 0–10 с обрезкой и разбором markdown, `build_prompt` со сценарием и хвостом диалога); `scoring::{TurnPoints, heuristic_points, blend, apply_points}` — `round((1−w)·эвристика + w·LLM)`, w = `scoring.llm_judge_weight` (default 0.4), бонусы и счётчики feedback остаются эвристическими; запрос судьи идёт **параллельно** ответу собеседника (`tokio::join!`), сбой любой природы (настройка выкл., квота, модель не настроена, таймаут 10 с, мусорный ответ) → откат на эвристику и `judge: null`; настройки `scoring.llm_judge_enabled`/`scoring.llm_judge_weight` в DEFAULTS и в админке; UI-бейдж `LLM s/a/t` в ленте диалога (ru/en i18n); DOCUMENTATION/ROADMAP обновлены. **215 тестов зелёные**, fmt/clippy `-D warnings` ✅ |
| 2026-09-25 | хакатон (LLM-судья, категория) | **Категория стратегии от модели**: судья возвращает `category` (`collaboration`/`compromise`/`confrontation`, слаги и русские названия), `apply_points(..., strategy)` ведёт счётчики по ней, в БД и `TurnOutcome.strategy_slug` пишется эффективная категория — бейдж под баллом теперь меняется от реплики к реплике и переживает перезагрузку (tooltip «Стратегия определена моделью: N/10»); неизвестная/отсутствующая категория → эвристика; таймаут судьи **10 → 20 с** (модель иногда отвечает дольше). **218 тестов зелёные**, fmt/clippy `-D warnings` ✅ |
| 2026-09-25 | хакатон (общие модели) | **Настройка моделей админа → все пользователи**: новый `GET /api/v1/model-preferences/effective` отдаёт фактически подключённую модель по ролям `llm/stt/tts` с источником `personal`/`global`/`none` (`ProviderService::effective_models`); личный кабинет (`#/settings`) показывает строку «Сейчас подключено: <модель> · общая настройка администратора», а у админа в той же секции появился селектор **общего назначения** (`PUT /model-assignments/:role`) с автосбросом его личного предпочтения по роли; раньше админ выбирал модель в личных настройках (audit: `model_preference.set`), глобальное назначение оставалось `demo-mock-llm` и все обычные юзеры ходили в демо-бота. Глобальное назначение `llm` переведено на `ds/deepseek-v4-flash` — обычный пользователь теперь реально разговаривает той же моделью (проверено: ответ модели + судья отвечает). **220 тестов** ✅ |
| 2026-09-26 | хакатон (конфигурируемость) | **Контекст генератора сценариев** (ТЗ п.2): `GenerateContext` в `application/scenario.rs` (brief + сфера, тема, сложность, роли и цели сторон, тон собеседника) → блок в user-промпте (`build_user_prompt`, маркеры «Сложность: … (slug)»/«Бриф:» сохранены для demo-mock) + **буквальное применение к результату** (`apply_context` — воля администратора приоритетнее полей LLM; пустые не затирают); `GenerateRequest` расширен опциональными полями (обратно-совместимо); UI-генератор: карточки «Контекст симуляции» и «Стороны (необязательно)» (сфера, тема, роли, цели, тон); unit-тесты промпта/применения + HTTP-тест буквального применения контекста через mock; **фильтр по сфере** в каталоге игрока (`scenarios.js` + i18n ru/en «Все сферы»); DOCUMENTATION: §2.8 «Конфигурируемость симуляции», фикс дрейфов таблицы маршрутов (`/scenarios/generate`, logout). **258 тестов зелёные**, fmt/clippy `-D warnings` ✅, `node --check` ✅ |
| 2026-09-26 | хакатон (презентация) | **Презентация** `docs/presentation.pptx` + `docs/presentation.pdf` (11 слайдов, ТЗ п.4: аудитория/проблема, ценность, механики и сценарии, путь пользователя, конфигурируемость, геймификация, техрешения, границы MVP, демо-скрипт; палитра из `app.css`, заметки докладчика на каждом слайде; генератор — `scripts/make_presentation.py`, python-pptx); ссылки в README/CONCEPT/ROADMAP |
